...
// Feature: Only 5 AI endpoints despite document-heavy domain
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_only_5_ai_endpoints_despite_document_heavy_domain',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'Only 5 AI endpoints despite document-heavy domain' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'Only 5 AI endpoints despite document-heavy domain'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_only_5_ai_endpoints_despite_document_heavy_domain', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_only_5_ai_endpoints_despite_document_heavy_domain', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_only_5_ai_endpoints_despite_document_heavy_domain', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_only_5_ai_endpoints_despite_document_heavy_domain']
    );
