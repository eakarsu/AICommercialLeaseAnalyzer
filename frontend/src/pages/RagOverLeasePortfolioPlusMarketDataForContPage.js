...
// Feature: RAG over lease portfolio plus market data for contextual negotiation advice
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverLeasePortfolioPlusMarketDataForContPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-lease-portfolio-plus-market-data-for-cont/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over lease portfolio plus market data for contextual negotiation advice</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-lease-portfolio-plus-market-data-for-cont/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
