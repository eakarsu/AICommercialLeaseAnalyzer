...
// Feature: White-label deployment for tenant-rep brokerages and corporate real-estate teams
import React, { useState } from 'react';
import api from '../services/api';

export default function WhiteLabelDeploymentForTenantRepBrokeragesAPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-white-label-deployment-for-tenant-rep-brokerages-a/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>White-label deployment for tenant-rep brokerages and corporate real-estate teams</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-white-label-deployment-for-tenant-rep-brokerages-a/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
