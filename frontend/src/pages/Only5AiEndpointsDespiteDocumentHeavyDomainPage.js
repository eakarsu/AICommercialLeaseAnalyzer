...
export default function Only5AiEndpointsDespiteDocumentHeavyDomainPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-only-5-ai-endpoints-despite-document-heavy-domain/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 5 AI endpoints despite document-heavy domain</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-5-ai-endpoints-despite-document-heavy-domain/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
