...
// Feature: Real-time critical-date alerts with auto-drafted notices
import React, { useState } from 'react';
import api from '../services/api';

export default function RealTimeCriticalDateAlertsWithAutoDraftedNPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-real-time-critical-date-alerts-with-auto-drafted-n/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time critical-date alerts with auto-drafted notices</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-critical-date-alerts-with-auto-drafted-n/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
