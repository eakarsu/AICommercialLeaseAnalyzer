...
// Feature: Agentic lease abstractor + reviewer extracting clauses, flagging risks, and suggesting redlines
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticLeaseAbstractorReviewerExtractingClausPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-lease-abstractor-reviewer-extracting-claus/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic lease abstractor + reviewer extracting clauses, flagging risks, and suggesting redlines</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-lease-abstractor-reviewer-extracting-claus/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
